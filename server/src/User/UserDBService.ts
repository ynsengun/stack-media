...
export class UserDBService {
    ...
    public async getFriendActivities(user: User): Promise<any> {
        ...
        let sqlQuery = "SELECT UserFriend.username as username, MAX(Watch.timeStamp) as lastActivity FROM ((SELECT friend1Username as username FROM Friendship WHERE friend2Username = '" + user.username + "') UNION (SELECT friend2Username as username FROM Friendship WHERE friend1Username = '" + user.username + "')) UserFriend LEFT OUTER JOIN Watch ON UserFriend.username=Watch.username GROUP BY UserFriend.username;";
